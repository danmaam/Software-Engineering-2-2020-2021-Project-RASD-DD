...
\section{Introduction}
	...
	\begin{itemize}
		...
		%Subsequently, the section ends with the most important phenomena that cannot be managed by the system.
	...
	The \emph{UML} does not include every class of the actual implementation of the system.
		...
		\subsubsection{UML Description}
			...
			\begin{itemize}
				...
				\begin{itemize}
					\item Pending: the reservation is in queue and hasn't been already called. So, called, entry and exit times are not present.
					\item Called: the reservation have been called to enter the store. Called time is present, but not entry and exit one, and the reservation is in store's called tickets.
					\item Discarded: the reservation have been called, but customer hasn't scanned his QR Code in time. Only call time is present, and the reservation isn't in any store reservation's list.
					\item Validated: the reservation entered, all the time are present but exit time if the customer hasn't already exited, and the reservation is in store's entered reservations. If the customer is still inside, the reservation in department reservations' inside list.
				\end{itemize}
		...
		\begin{figure}
			\begin{adjustwidth}{-3cm}{}
				\centering
				\includegraphics[scale=0.4, angle=90, trim= 0 0 0 -5cm]{ClassDiagrams/classDiagram.pdf} \\
...
The same doesn't applies to requests of travel times and for mail sending, since they are achieved by \emph{APIs} of some external services.
		...
		\begin{itemize}
			\item {\bfseries Customer Device:} Customer needs to have a device with a working network module, a \emph{GPS} module and a high resolution display for the QR-Code to be scanned.
			...
			\item {\bfseries \emph{QR Code} readers:} \emph{QR Code} readers must have a working network module, and a optical scanner to scan QR Codes. 
		\end{itemize}
