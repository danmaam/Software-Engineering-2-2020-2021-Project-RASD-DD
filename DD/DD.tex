		...
		\begin{center}
			...
			\begin{adjustwidth}{-2.3cm}{}
				\begin{tabular}[h!]{|m{4em}|m{5em}|m{26em}|}	
					...
					\hline	
					\xrowht{5pt}
					\centering 1.1 & 04/01/2021 & Mock-up and software architecture revision \\
					\hline	
					\xrowht{5pt}
					\centering 1.2 & 09/01/2021 & Final revision \\
					\hline	
					\xrowht{5pt}
					\centering 2.0 & 10/01/2021 & Second version \\
					\hline	
				\end{tabular}
			\end{adjustwidth}
...
\section{Architectural Design}
	...
	\begin{figure}[!h]
		\begin{adjustwidth}{0cm}{}
			\centering
			\includegraphics[scale=0.5]{../UML Diagrams/CompositionDiagram/CompositionDiagram.pdf} \\
		\end{adjustwidth}
	...
	\subsection{Component View}
		...
		Here, there is described the adapter for \emph{DBMS}, since it’s the most articulated among those.
			...
			As said before, it manages customers’ requests and access operations, through the two interfaces it exposes:
			...
			\paragraph{Reservation Handler Component}
				...
				\begin{itemize}
					\item Suggestions
					\item Management of Customer's requests on reservations
				\end{itemize}
			
				The component uses {\bfseries ReservationHandlerInterface} (that implements \emph{QueryInterface} and \emph{SuggestionInterface}) to accept requests, and {\bfseries ReservationInterface}, used to make requests to a specific store.
				...
				\begin{itemize}
					\item {\bfseries SuggestionComponent}: it is the component that manages the suggestions that are made available to the customer at the time of reservation. It will query each store registered on the platform, to retrieve availability information, depending on the preferences inserted by the customer and the type of reservation they are making (so, the component will use the right {\bfseries Query Component}); it’s invoked by both the \emph{ReservationHandlerInterface} when a customer asks for suggestions on bookings, or by {\bfseries ASAPQueryComponent} while the \emph{ETA} for entering the store \emph{ASAP} is too high.
					
					\item {\bfseries StoreQueryComponent}: it is the component that manages the queries that are made at a particular store, indeed this component uses the {\bfseries ReservationInterface} to interact with the right {\bfseries StoreComponent}. The queries can be of two types:
					
					\begin{itemize}
						\item {\bfseries ASAPQueryComponent}: it is the component that manages \emph{ASAP} reservations, asks the server waiting times for accessing the store, notifies the store if a reservation is made and manages the deletion of \emph{ASAP} reservations.
		...
		\subsubsection{Store Component}
			The Store is the component that manages the store from all points of view and it is the most important component of the whole architecture, since implements the greatest part of the  system's \emph{Business Tier}. The component analyzes many important aspects, but the most one are:
			...
			\begin{itemize}
				...
				\item Manage Building Flow
			\end{itemize}
		
			As said before, it’s been preferred to separate the logic governing each store by assigning a different component for each different store. Also here, the store component have been designed in a modular way, to separate the different features offered by a single store; these are:
			
			\begin{itemize}
				\item {\bfseries FlowComponent}: this component manages the functionalities that are requested when a \emph{QR Code} is scanned at the store entry, and uses the {\bfseries CallingNumberComponent} to check the scanned \emph{QR Code} status, the {\bfseries DepartmentComponent} to register the entrance/exit and the {\bfseries StatisticComponent} to update visit statistics.
				
				\item {\bfseries StatisticComponent}: it is the component dedicated to building and take trace about customer's visit statistics. It’s used by:
				
				\begin{itemize}
					\item {\bfseries FlowComponent}: in order to update visiting statistics at customers’ exit
					\item {\bfseries ReservationComponent}: invokes its methods when, during a reservation process, it’s necessary to infer the estimation of customers’ shopping time.
				\end{itemize}
			
				\item {\bfseries RearrangeComponent}: it is the component that rearrange the reservations when a store manager modifies the capacity of some part of the store, or some reservations are changed, and the queue must be reschedules. Interacts with the {\bfseries ReservationComponent} in order to fix store's situation. In case of modifications, the customers will receive a notification.
				
				\item {\bfseries StoreDataManager}: it is the adapter between the store and its dedicated \emph{DBMS}. The choice of dedicating a \emph{DBMS} for each store is due to efficiency and security reasons: if a \emph{DBMS} breaks, it wouldn't affect the whole system, while a store can access only its data. Though, for each store is faster to query only on its data.
				
				\item {\bfseries ParameterManager}: it is the component that manages the parameters, the settings and the situation of each department of the store. It’s used by:
				
				\begin{itemize}
					\item {\bfseries FlowComponent}: to update Departments’ situation.
					\item {\bfseries StoreManagerHandler}: to update store parameters, and to access the real-time situation of the store.
					\item {\bfseries ReservationComponent}: to request store situation in order to make a reservation.
					\item {\bfseries RearrangeComponent}: when a parameter is modified, to make coherent the store’s status.
				\end{itemize}
		
				
				\item {\bfseries ReservationComponent}: it's the component dedicated to the management of reservations. It’s used to retrieve reservations made in the past, to retrieve waiting times, and to memorize customers’ reservations. Uses the {\bfseries CallingNumberdComponent} to notify a modification in reservations list (so that the queue can be rearranged), and it’s used by the latter to initialize the queue at store opening. It also handles modifications made by customers and store managers to reservations. Furthermore, a store manager uses it to contact customers about some specific reservation. Because of this, the components interact with {\bfseries NotificationComponent} and {\bfseries Mail Component} to interact with customers. 
				...
				\item {\bfseries CallingNumberComponent}: it is the component designated to manage store's queue and to call in reservations. It’s divided in two subcomponents, each carrying out different functionalities:
				...
				\begin{itemize}
					...
					\item {\bfseries CallNumberComponent}: takes a queue of reservations to be called, and when it’s possible, a reservation is called to enter. It’s alerted by the {\bfseries FlowComponent} that somebody exited the store.
				\end{itemize}
			\end{itemize}
		
			The core store functionalities are requested through the \emph{ReservationInterface} (dedicated to customers’ features), \emph{StoreInterface} (dedicated to store managers), \emph{ScannerInterface} (dedicated to QR Scanners) and \emph{TotemInterface} (dedicated to totems); the requests of the last three requests pass through an handler, since arrives from a client. Here there is described the one for \emph{store managers}. 
		...
		\subsubsection{Store Manager Handler Component}
			...
			This component handles all the possible settable options of a store manager, it's invoked by methods defined on the \emph{StoreManagerHandlerInterface} from the mobile application, and uses the \emph{ParameterInterface} and \emph{ReservationInterface}, to manage all functionalities regarding parameters and reservations.
			...
			\begin{itemize}
				...
				\begin{itemize}
					...
					\item {\bfseries StoreSettingManager}: it is the component that manages all the parameters which concern the entire store. Indeed, the store manager, can change the maximum capacity of simultaneously allowed booked people inside the store and can modify the opening and closing hours.
				\end{itemize}
	...
	Since the application needs to remotely communicate with various actors, the architecture which best fit for \emph{CLup} purposes is the {\bfseries client-server} one. Moreover, this architecture has been implemented through the {\bfseries three-tier} paradigm. This aims to separate the different logic part of the system: {\bfseries presentation}, {\bfseries business} and {\bfseries data}.
	...
	\begin{itemize}
		\item {\bfseries Mobile App}: it's the mobile application used by \emph{customers} and \emph{store managers} to request services to \emph{CLup}. It also represents the {\bfseries Presentation Tier} of the system, developed following the \emph{thin client} paradigm, leaving the whole business logic to the server, but for a thing: the notifications when a customer should depart for the store, and the map related functions. In fact, it's the \emph{Mobile App} that monitors when a it's time to go to the store. Even if a \emph{thin client} shouldn't handle operations, this aims to reduce the work load of the server: it's useless to remotely request the customer's position, and then send it to the \emph{Map Service}, while mobile devices can do it on their own (the same applies for distance and travel time); to make everything work properly, if the \emph{ETA} of some reservation drastically changes, the \emph{Mobile App} is alerted through a \emph{notification}.
		\item{\bfseries QR Code Scanner}: it's role is to send server the scanned \emph{QR Code} to process it;
		\item{\bfseries Totem}: a client needed to allow making requests physically at any store; 
	...
	\begin{itemize}
		\item {\bfseries Business-logic}: it's implemented by {\bfseries CLupServer} and {\bfseries StoreComponent}. The first is needed to manage users access to \emph{CLup} services and reservations request, forwarding them to the correct store, or to generate shopping suggestions by querying all the stores managed by the system. Instead, \emph{StoreComponent} manages the business logic related to each store, including reservations, ticket calling, capacity ecc. This design choice is to separate the logic of each store, and to avoid disservices to the whole \emph{CLup} system if something may fail.
	...
	In this section will be analysed the runtime view of the system, through some \emph{Sequence Diagrams}. This should help to understand how the different components interact. Some aspects have been simplified to make the diagrams more readable; e.g., some interactions with the \emph{DBMS} and between adapters and external services have been omitted with some error messgaes, and some operations with identical behavior have been collapsed in generic requests, through a generic method, just to model the interaction. The \emph{Component Interfaces diagram} will report all these methods in detail.
	...
	Then, after the customer select the store, the departments, and insert the shopping time estimation (here it's not treated the calculation by the \emph{CLup} since it differs only for a message sent to the {\bfseries StatisticComponent} of the correct store by the {\bfseries ReservationComponent}), they can both make an \emph{ASAP reservation}, or a \emph{Booking}.
	\begin{figure}[H]
		\begin{adjustwidth} {-1cm}{}
			\centering
	...
	\end{figure}
	When the customer open the app's area reserved for made reservations, and there is an internet connection, the \emph{Mobile App} requests the server for all the reservations made by the logged customer; the process is mainly handled by the {\bfseries ReservationHandler}, that queries all the stores for the reservations. Once this process is completed, the customer have a list of reservations. When someone is opened, the app requests the travel time for the store to the MapManager, and, in case of an \emph{ASAP} reservation, the new \emph{ETA} to enter the store at the {\bfseries ReservationHandler} (which forwards the request to the correct store component).\\\\
	...
	A note on these sequence diagram: to make them more compact, the interactions with the {\bfseries StoreManagerHandler} have been avoided, since it's only a redirector from the \emph{Mobile App} to the correct component of the {\bfseries StoreComponent}.\\
	In order to manage on reservations, these must be retrieved from the server. The request is received from the {\bfseries ReservationComponent}, that queries the \emph{DBMS} for all the made reservations. Once retrieved the store's reservation, a manger can both contact customers about a specific one, or modify something of a specific reservation (such as deleting or rescheduling it)\\\\
	...
	Here there are described the actions made by the system to handle the process for calling a customer to get inside the store. This process is mainly triggered when the queue is modified in some way, such as someone exits the store, some reservation has been rescheduled or cancelled or someone didn't enter the store in time, and their reservation has been discarded. \\\\
	First of all, the {\bfseries CallingNumberComponent} process the status of the queue, getting the first reservation that has the right to be called. Then, the {\bfseries ParameterManager} is asked for the current situation of the store, and the response is sent. In case there is the possibility to allow the considered customer to enter the store, the reservation is set as \emph{called}, this status registered in the \emph{DBMS}, and the call is notified to the {\bfseries NumberCallingApp}, so that outside the store the number can be called. Finally, the system checks if the \emph{ETA} for some future reservation decreased, and if so sends a notification to the related customer's \emph{Mobile App}, so that the calculation of the departure time is precise. Even if not described in other diagrams, this last process takes place each time a modification in the queue is done.\\\\
	This process runs until the store is full and/or no other reservation can enter the store.
	...
	The last process to be analyzed is the notification when a customer should depart to get in time, and not too late to the store. As described before, this operation is entirely handled by the MobileApp: if this is processed by the server, it would be necessary to continuously request to the client the \emph{GPS} Position, forward it to Map Service to calculate the travel time, and if necessary send to the customer a notification. This may overload the server of useless traffic, since this operation can be handled by the MobileApp, directly accessing to the map external service and the \emph{GPS Module} of the mobile device running it.
	\begin{figure}[H]
		\begin{adjustwidth} {-1.8cm}{}
			\centering
	...
	\end{figure}
	The previous diagram represents the interfaces of the most important and critic components of witch \emph{CLup} is composed. All of these interfaces contains the methods used in the previously described \emph{Sequence Diagrams}, and shows how the methods of a \emph{component} implementing a specific \emph{interface} use other parts of the system to complete their tasks. Not all the interfaces are described here, such as the one related to the handlers, since they are only adapters between {\bfseries Presentation Tier} and {\bfseries Business Tier}, so representing them would be only a repetition of the interface they adapt.
	As \emph{Sequence Diagrams} were simplified, here the methods' types and parameters are totally fictitious, and used only to gave an idea of the content transmitted from a component to another.
	\begin{itemize}
		...
		\item {\bfseries StoreManagerHandlerInterface}: represents all the functionalities that a \emph{Store Manager} can access using the \emph{MobileApp}. Since the main things that a manager can do is to manage reservations and settings of the store, this interface implements the two interfaces relative to the {\bfseries ParameterComponent} and to the {\bfseries ReservationComponent}. The first expose all the methods necessary to manage the settings of the store the second is related to the management of store's reservation, and expose all the methods necessary to access and manage store's reservation. These interfaces are usable also by other components that need to get the settings or the reservation history of the store.
		...
		\item{\bfseries Data Interface}: exposes the methods needed by the components of the {\bfseries CLup Server} to access data necessary to do the business tasks related to the system, such as checking and storing credentials in respectively login and registration process, and to memorize the stores managed by the system.
	...
	As briefly introduced before, since \emph{CLup} is a service usable by a great amount of people, and all the operations made trough the system must be considered in every request made by some \emph{client}, the architectural style best fitting for the described service is the {\bfseries client-server} one. In fact, there are clients that make requests to a server, that processes them and reply to clients with the response of each computation.\\
	In implementing the {\bfseries client-server} architecture many alternatives have been analyzed, but at the end the more suitable for \emph{CLup} scope is a {\bfseries three tier architecture}, that, as previously said and described, logically separates the {\bfseries presentation}, {\bfseries business} and {\bfseries data} tier.\\
	A particular description must be done about the particular {\bfseries business} and {\bfseries data} tier here implemented. In fact, in the system there isn't a unique principal business component, and/or a unique data tier. Since each store is independent from the other, and should access only to data of its competence, the business tier is divided so that each store has its own business logic. This helps in various situations, such as greater flexibility in accepting managers requests to implement a certain business logic, maybe different from the one of some other store; in case of disservices, only the broken store won't work, while the components of the others will work, and it's easier to isolate the broken one and repairing it without touching anything else; each store's {\bfseries business tier} has an associated and unique {\bfseries data tier} to access: this allows to separate each store's data, in order to make data consultation faster and more secure, since each store can access only to the associated \emph{DBMS}.\\
	\begin{figure}
		\begin{adjustwidth} {0cm}{}
			\centering
	...
	\end{figure}
	An advantage of this is that each \emph{client} that handles operations on a single store, can communicate directly to the correct {\bfseries business tier}. Instead, customers connect to a dedicated {\bfseries business tier} with its own {\bfseries data tier}; this redirects requests from customers to a specific store, and generate suggestions querying all the possible stores, each of them present in the general \emph{DBMS}.
	...
	\begin{itemize}
		\item {\bfseries Pattern Adapter}: used in multiple part of \emph{CLup}. Each external service is accessed though an adapter that takes requests, and redirect them to the adapted service. This aims to easily implement multiple external service or to change them without a great effort.
		\item{\bfseries Model View Controller}: used in each client developed for the system.
		\item{\bfseries Facade Pattern}: allows the {\bfseries StoreManagerHandler} to access more easily to multiple components trough the different interfaces they implement.
	\end{itemize}
...
In this section it is described how the requirements of the application are mapped to the components defined previously. Notice that here there are described only the components which initiates the process associated to verify the requirement (excluding the various \emph{client handlers}). The \emph{Runtime View} already specifies which other components are necessary to complete the requests.
...
\section{Implementation, Integration and Test Plan}
	...
	After have considered the various options available to define a \emph{Implementation and Testing Plan}, it's been decided to use the {\bfseries bottom-up approach}. So, for each major component, the development starts from the subcomponents with less dependencies, and so on. Moreover, our system has been created with the intention of preserving modularity, in order to have loose coupling between components, thus simplifying future changes and maintenance. In the described plan, the components that can be simultaneously developed are grouped, to give an idea how the work can be managed in order to save time in the {\bfseries IIT} process.
	The first two components that must be implemented and integrated are {\bfseries StoreComponent} and {\bfseries CLupServer}.
		...
		Before defining the {\bfseries IIT} of the {\bfseries Store Component}, a \emph{Use Relation Hierarchy Diagram} have been traced, to understand the dependencies between the various components, so that an order of implementation can be defined.
		...
		\begin{enumerate}
		\item The components that have no other dependencies to others are {\bfseries DataManager}, {\bfseries MailComponent}, {\bfseries NotificationManager}. So, they are the first components that will be implemented. In particular,  {\bfseries DataManager} will be implemented after the installation of the \emph{DBMS}, so that, after unit testing it, it's possible to integrate it with the \emph{DMBS} and test the integration. Instead, for {\bfseries NotificationManager} and {\bfseries MailComponent} it's not possible to go on with the integration, but the process it's limited to the of use some \emph{Drivers} in the \emph{unit testing} phase.
		\item Then, it's easy to implement the {\bfseries StatisticComponent}. After unit testing it, it's possible to integrate, and test the integration with {\bfseries DataManager}, but it's not possible to integrate it with the {\bfseries ParameterManager} or the {\bfseries FlowComponent}, not developed at this point, but it's possible to use some \emph{Drivers} and \emph{Stubs} (the latter just for {\bfseries ParameterManager} just to complete the unit testing; the implementation test will be done later. 
		\item So, it's possible to develop the {\bfseries CallingNumberComponent} and {\bfseries Parametermanager} and, after the \emph{unit testing} phase, integrate, and test the integration, with the {\bfseries StatisticComponent} and the {\bfseries DataManager}. Since some dependencies are not have been completed at this point, in this phase the integration test can't be complete, but only the \emph{unit testing} can be finished, with the need for \emph{Stubs} simulating {\bfseries RearrangeComponent} and {\bfseries NumberCallingApp}, and \emph{Drivers} simulating {\bfseries ReservationComponent}, {\bfseries StoreManagerHandler};
		\item At this point, it's possible to start the development of the {\bfseries ReservationComponent}. After completing the development and the unit testing, it's possible to integrate, and test the integration with {\bfseries ParameterManager}, {\bfseries CallingNumberComponent}, {\bfseries MailComponent} and {\bfseries DataManager}. As in the previous phase, for the non-implemented components, only unit testing is conducted, with some \emph{Drivers} to simulate {\bfseries StoreManagerHandler}, {\bfseries RearrangeComponent} and calls from components implementing {\bfseries ReservationInterface}.
		\item After the previous phases these have been completed, it's possible to implement, test and integrate {\bfseries FlowComponent}, {\bfseries RearrangeComponent} and {\bfseries StoreManagerHanlder} with the other components already developed; it's needed only a \emph{Driver} simulating {\bfseries Scanner App} for completing the unit testing of {\bfseries FlowComponent}; their integration test it's still not possible.
		 \end{enumerate}     
		...
		Also for {\bfseries CLupServer} a \emph{Use Relation Hierarchy Diagram} have been done, to identify the order to implement the single components.
		\begin{itemize}
		\item The first components that can be implemented are {\bfseries MailComponent} and {\bfseries DataManager}; as for the {\bfseries StoreComponent}, the {\bfseries DataManager} should be implemented, tested and integrated after the associated \emph{DBMS} has been installed. Here it's not possible to integrate other things, only the unit testing of the components are possible (but the \emph{DBMS} with the {\bfseries DataManager}.
		\item Then, it's possible to implement the {\bfseries AccessManager} and the {\bfseries ReservationHandler}, which, after their \emph{unit testing}, can be integrated and tested with the {\bfseries DataManager}. For the components not already developed, the integration is not done, but only the unit testing though some \emph{Drivers} simulating {\bfseries CustomerHandler} and {\bfseries MobileApp} and the {\bfseries StoreComponent}.
		\item Finally, the {\bfseries CustomerHandler} can be implemented, integrated with the {\bfseries ReservationHandler} and tested. The only component not already integrated is the {\bfseries MobileApp}, so it's possible only to unit test the interactions with a \emph{Driver} to simulate it.
		\end{itemize}
		
		After the previously described components have been developed, integrated and unit tested, it's possible to develop all the clients, \emph{unit-testing} them and integrate, and test the integration, of the whole system.
	...
	\begin{itemize}
		...
		\item \textbf{Performance Testing}: highlights problems related to the performance and bottlenecks; this test aims to check that the time constraint introduced in the RASD are respected, such as \emph{QR Codes} processing time, the time needed to call a new reservations and that needed to process a new one, general system responsiveness and departure notification generated in time.
		\item \textbf{Load Testing}: highlights problems memory-related such as memory leaks and buffer overflows, e.g. verify if the system works even with a large number of users operating at the same time, respecting the constraints of simultaneous users that the system must be able to manage, specified in the RASD.
		\item \textbf{Stress Testing}: ascertains that the application recovers in the right way after failure (e.g. see what happens after an unexpected internet disconnection during an operation, and how the system responds to recover the data and operativeness after a failure of some component)
	\end{itemize}
